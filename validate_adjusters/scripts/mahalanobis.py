...
def mahalanobis_distances(
    ...
    bayesian_params: dict[str, dict],
) -> pl.DataFrame:
    
    """Compute Mahalanobis distance between adjuster parameters and the Bayesian posterior.

    For each gene present in all three inputs, computes:

        d = sqrt( (x - mu)^T @ Precision @ (x - mu) )

    where x = [adjuster_scale, adjuster_shift] and mu is the Bayesian posterior mean.
    ...
    bayesian_params : dict[str, dict]
        Output of parse_bayesian_params. Each value has keys "mean" (shape (2,))
    ...
    genes = sorted(
        set(adjuster_scales) & set(adjuster_shifts) & set(bayesian_params)
    )

    rows = []
    for gene in genes:
        x = np.array([adjuster_scales[gene], adjuster_shifts[gene]])
        mu = bayesian_params[gene]["mean"]
