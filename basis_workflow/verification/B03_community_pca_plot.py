#!/usr/bin/env python3
"""
PCA on genes from ESR1 and ERBB2 communities only, before/after alignment.

Outputs:
  community_pca_alignment.png — 2x2 grid: before/after × dataset/subtype
"""
...
def main():
    ...
    # Load gene sets and find ESR1/ERBB2 communities
    with open(args.gene_sets) as f:
        gene_sets = json.load(f)

    restrict_genes = set()
    for axis_name, genes in gene_sets.items():
        if "ESR1" in genes or "ERBB2" in genes:
            restrict_genes.update(genes.keys())
            print(f"  Including {axis_name} ({len(genes)} genes): {'ESR1' if 'ESR1' in genes else 'ERBB2'}")
    print(f"  Total restricted gene set: {len(restrict_genes)} genes")
    ...
    common = sorted(set(ga.columns) & set(gb.columns) & set(gc.columns) & restrict_genes)
    ...
    plt.suptitle(f"PCA on ESR1 + ERBB2 community genes ({len(common)} genes) — before vs after", fontsize=14)
    plt.tight_layout()
